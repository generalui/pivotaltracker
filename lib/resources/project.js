...
function _callbackWithSearchResults(error, res, cb) { // cb(error, results)
    if (error) {
        ...
        for (var key in res.data) {
            ...
            // Yes, this is awkward. The value of the key 'stories' 
            // is expected to be an object with another key called 'stories'
            if (key === 'stories' && 
            Array.isArray(res.data[key].stories) && 
            res.data[key].stories.length) { 
                           
                res.data[key].stories.forEach(function(s) {
                    if (s) {
            ...
            if (key === 'epics' && 
            Array.isArray(res.data[key].epics) && 
            res.data[key].epics.length) { 
                           
                res.data[key].epics.forEach(function(e) {
                    if (s) {
