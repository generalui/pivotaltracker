...
function upload(token, pathSegments, filepath, cb) { //cb(error)

    var path = Array.isArray(pathSegments) ? pathSegments.join('/') : '';
    var url = 'https://www.pivotaltracker.com/services/v5/' + path;
...
function download(token, pathSegments, filepath, cb) { //cb(error)

    var path = Array.isArray(pathSegments) ? pathSegments.join('/') : '';
    var opts = {
        url: 'https://www.pivotaltracker.com/' + path,
        headers: {
            'X-TrackerToken': token
        }
    };
