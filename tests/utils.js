                    ...
                    if (typeof args[i] === 'object' && args[i] !== null) {
                        for (var key in args[i]) {
                            if (args[i].hasOwnProperty(key)) {
                                ...
                                test.equal(obj1[key], args[i][key], msg);
                            }
                        }
                    }
                }
...
exports.standardPropertyTestFunction = function(constructor,propertyName,happyTestValue) {
     return function(test) {
        ...
        test.ok(newObj[propertyName] === happyTestValue, propertyName+' should be equal to value passed into setter if a string or null value. Current value: '+newObj[propertyName]);
        
        /* Happy path test, set to null */
        newObj[propertyName] = null;
        test.ok(newObj[propertyName] === null, propertyName+' should be able to be set to null. Current value: '+newObj[propertyName]);
        
        /* Negative test, assign a function value to non-function property */
        var functionVal = function(){};
        newObj[propertyName] = functionVal;
        test.ok(typeof newObj[propertyName] !== 'function', propertyName+' should not be able to be set to a function. Current value: '+newObj[propertyName]);
...
exports.standardDatePropertyTestFunction = function(constructor,propertyName,happyTestValue) {
     return function(test) {
        test.expect(4);
        var newObj = new constructor();
        ...
        test.ok(date && newObj[propertyName] && date.getTime() === newObj[propertyName].getTime(), propertyName+'.getTime() should be equal to testValue.getTime() passed into setter if not null. Current value: '+newObj[propertyName]);
